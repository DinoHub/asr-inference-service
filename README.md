# Singlish ASR Inference Service

Singlish-Accented ASR Inference Service for AI App Store
...
```sh
docker tag singlish-asr-inference-service:1.0.0 <REGISTRY>/<REPO>/singlish-asr-inference-service:1.0.0
```
...
```sh
docker push <REGISTRY>/<REPO>/singlish-asr-inference-service:1.0.0
```
...
2. Pass the docker image URI (e.g `<REGISTRY>/<REPO>/singlish-asr-inference-service:1.0.0`) when creating/editing the inference service
